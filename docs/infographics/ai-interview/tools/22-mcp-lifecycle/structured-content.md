...
MCP 生命周期分初始化、正常操作和关闭。Client 先发送 initialize，包含自己支持的 protocolVersion、capabilities 和 implementation 信息。Server 若支持请求版本就回同一版本，否则回它支持的另一个版本。Client 不支持 Server 所选版本时应断开；
...
连接池按 Server、身份、协议版本和会话隔离。保存协商结果，发送每个请求前检查能力。Streamable HTTP 的 SSE 流中断后，Client 可在同一会话中携带 `MCP-Session-Id`，并用 `Last-Event-ID` 恢复消息，无需重新初始化；
